...
classifier.py  Stage 1: BL_COMPARISON / SI_REQUEST / INVOICE_QUERY /
...
## Notes
- The `data/` folder is a local copy of the bundle (`inbox/`, `attachments/`)
  plus `ground_truth.json` used only for offline evaluation / regression —
  answer keys are never baked into the submitted output.
- `classifier.py` and `comparator.py` rely on the template structure of the
  generated bundle; if the live dataset diverges, re-tune the keyword rules
  (`classifier.py`) and extend `extractor._LABEL_ALIASES` / `formats.py`.